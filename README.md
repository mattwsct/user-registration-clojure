...
* [`src/registration_app/`](src/registration_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/registration_app/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `registration-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/registration_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns registration-app.example
  (:require [registration-app.config :as config])
